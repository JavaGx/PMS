...
<form id="J_Form" class="form-horizontal" method="post" action="">
		...
		<div class="control-group"> 
			     ...
			     <div class="controls"> 
			      <input id="name" type="text" class="input-normal control-text" style="width:100px" name="name" data-rules="{required:true}"/> 
			     </div>
			</div>
		<div class="control-group"> 
			     <label class="control-label">
			     ...
			     <div class="controls"> 
			      <input id="url" type="text" class="input-normal control-text" style="width:200px" name="url"/> 
			     </div>
			</div>
			<div class="control-group">
                ...
                <select id="parentId" name="parentId">
                  ...
                  <c:if test="${parents!=null}">
                  	<c:forEach items="${parents}" var="privilege">
                  		<option id="${privilege.id}" value="${privilege.id}">${privilege.name}</option>
                  	</c:forEach>
                  </c:if>
                </select>
              </div>
			...
			<div class="search-grid-container">
				<div style="margin-left: 30px;margin-top: 10px;"><h3>权限列表</h3></div>
...
<script type="text/javascript">
    ...
    BUI.use(['bui/grid','bui/data'],function(Grid,Data){
	    ...
	    columns = [
		    ...
		    { title: '权限名称', width: 100, sortable: false, dataIndex: 'name', selectable: true },
		    { title: 'url', width: 100, sortable: false, dataIndex: 'url', selectable: true },
		    { title: '上级部权限', width: 100, sortable: false, dataIndex: 'parent', selectable: true,renderer:function(value){
		    		if(value!=null){
		    			return value.name;
		    		}else{
		    			return "没有上级权限";
		    ...
		    { title: '描述', width: 100, sortable: false, dataIndex: 'description', selectable: true}
	    ];
	    data = ${privileges} 
    ...
    function show () {
      ...
      var parent=selections[0].parent;
      ...
      if(selections.length==1){
         BUI.use(['bui/overlay','bui/form'],function(Overlay,Form){
		    var dialog = new Overlay.Dialog({
			    ...
			    success:function () {
				    $("#J_Form").attr("action","${path}/privilege/updatePrivilege.do");
					$("#J_Form").submit();
			    }
		    });
